...
function util.get_current_command_map_of_windows()
  ...
  local output = tmux_command "list-panes -a -F '#{session_id} #{session_name} #{window_id} #{window_name} #{pane_current_command}'"
  local map = {}
  for line in output:gmatch '[^\r\n]+' do
    local session_id, _, window_id, _, pane_current_command = line:match '^(%$%d+) (.+) (%@%d+) (.+) (.+)$'
